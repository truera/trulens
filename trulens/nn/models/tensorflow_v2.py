...
from trulens.utils.typing import DATA_CONTAINER_TYPE, many_of_om, nested_cast
...
class Tensorflow2ModelWrapper(KerasModelWrapper
    ...
    def _get_output_layer(self):
        ...
        for output in self._model.outputs:
            for layer in self._layers.values():
                try:
                    if layer is output or self._get_layer_output(layer) is output:
    ...
    def _is_input_layer(self, layer):
        if (self._model.inputs is not None):
            return any([inpt is self._get_layer_output(layer) for inpt in self._model.inputs])
        else:
            return False
