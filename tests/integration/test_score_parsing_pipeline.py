...
class MockLLMProvider(LLMProvider):
    ...
    def __init__(self, mock_response: str, **kwargs):
        super().__init__(
            endpoint=MockEndpoint(name="mock_endpoint"),
...
@pytest.mark.parametrize(
    "name, response_text, expected_normalized_score", test_cases
)
def test_score_parsing_pipeline(name, response_text, expected_normalized_score):
    ...
    result = provider.relevance(
        prompt="What is the question?", response="This is the answer"
    )
