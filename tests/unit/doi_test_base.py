...
class DoiTestBase(object):
    ...
    def setUp(self):
        ...
        self.z = get_backend().as_tensor(np.array([[1., 2., 3.], [0., -1., -2.]]))
    ...
    def test_point(self):
        ...
        self.assertTrue(
            np.array_equal(get_backend().as_array(res[0]), get_backend().as_array(self.z)),
            'Value of point should not change')

    # Tests for LinearDoI.

    def test_linear(self):
        doi = LinearDoi(baseline=np.ones(get_backend().int_shape(self.z)), resolution=21)
        res = doi(self.z)
        ...
        self.assertTrue(
            np.all(get_backend().as_array(res[-1]) == 1.), 'Last point should be baseline')

        self.assertTrue(
            np.allclose(
                get_backend().as_array(res[-2]),
    ...
    def test_linear_default_baseline(self):
        ...
        self.assertTrue(
            np.all(get_backend().as_array(res[-1]) == 0.),
    ...
    def test_gaussian(self):
        ...
        self.assertEqual(get_backend().int_shape(res[0]), get_backend().int_shape(self.z))

    def test_gaussian_non_tensor(self):
        doi = GaussianDoi(var=1., resolution=10)
        res = doi(get_backend().as_array(self.z))

        self.assertEqual(
            len(res), 10, 'GaussianDoi should return `resolution` points')

        self.assertEqual(res[0].shape, get_backend().int_shape(self.z))
