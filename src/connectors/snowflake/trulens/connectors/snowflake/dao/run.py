...
class RunDao:
    ...
    def call_compute_metrics_query(
        ...
        try:
            ...
            sql_cmd.collect()  # metric computation itself is asynchronous and managed by tasks, but we need to wait at least for query to return so that if the eval runs in a Python script, the processdoesn't exit before the query finishes setting up tasks.
        except Exception as e:
            logger.exception(
