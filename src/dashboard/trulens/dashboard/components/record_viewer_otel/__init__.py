...
def record_viewer_otel(spans, key=None) -> str:
    """Create a new instance of "record_viewer", which produces a timeline

    Args:
        spans: JSON of the record serialized by `json.loads`.

    Returns:
        TODO(garett): return ids instead of start time
    ...
    global _record_viewer

    if _record_viewer is None:
        if not _RELEASE:
    ...
    component_value = _record_viewer(spans=spans, key=key, default="")
