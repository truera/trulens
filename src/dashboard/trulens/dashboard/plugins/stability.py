"""Stability analysis plugin for the TruLens dashboard."""
...
def _get_stability_data(app_ids: List[str]) -> pd.DataFrame:
    ...
    if not stability_base_metric_columns:
        st.warning(
            "No stability base metric found. Please ensure you have "
            "QA_ACCURACY or INSIGHT_ACCURACY feedback defined."
        )
        return pd.DataFrame()
    ...
    def unique_input(series):
        if series.nunique() > 1:
            raise ValueError(
                "Not all values in the 'input' column are the same for "
                "this group."
            )
...
def render_stability(app_name: str):
    ...
    if stability_df.empty:
        st.warning(
            ...
            "QA_ACCURACY or INSIGHT_ACCURACY feedback."
        )
        return
...
# Register the stability page
register_page("Stability", render_stability)
