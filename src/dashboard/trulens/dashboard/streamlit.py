...
def trulens_leaderboard(app_versions: Optional[List[str]] = None):
    """
    Render the leaderboard page.

    Args:

        app_versions List[str]: A list of application IDs (default is None)
    ...
    df.sort_values(by="app_version", inplace=True)

    if df.empty:
        st.write("No records yet...")

    if app_versions is None:
        app_versions = list(df.app_version.unique())

    for app_version in app_versions:
        app_df = df.loc[df.app_version == app_version]
        if app_df.empty:
            continue
        app_str = app_df["app_json"].iloc[0]
        app_json = json.loads(app_str)
        metadata = app_json.get("metadata")
        st.header(app_version, help=draw_metadata(metadata))
