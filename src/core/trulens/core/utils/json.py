...
def jsonify(
    ...
    """Convert the given object into types that can be serialized in json.
        ...
        Args:
            ...
            redact_keys: redact secrets from the output. Secrets are determined by
    ...
    if isinstance(obj, Enum):
        ...
        if isinstance(obj, serial_utils.Lens):  # special handling of paths
            return obj.model_dump()

        forward_value = {}
        new_dicted[id(obj)] = forward_value
        forward_value.update({
            k: recur(pyschema_utils.getattr_serial(obj, k))
        ...
        forward_value.update({
            f.name: recur(pyschema_utils.getattr_serial(obj, f.name))
