...
class DatabaseVersionException(Exception):
    ...
    def behind(cls):
        ...
        return cls(
            ...
            "Please upgrade it by running `session.migrate_database()` "
            "or reset it by running `session.reset_database()`.",
            cls.Reason.BEHIND,
        )
    ...
    def reconfigured(cls, prior_prefix: str):
        ...
        return cls(
            ...
            f'Please update it by running `session.migrate_database(prior_prefix="{prior_prefix}")`'
            " or reset it by running `session.reset_database()`.",
