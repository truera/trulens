...
One constraint carries over directly from the privacy model above, and it's the one that actually determines whether these evals are useful: each capture flag gates its own fields independently, and they're all off by default. `TRULENS_CAPTURE_CONTENT` only restores `RECORD_ROOT.INPUT`/`OUTPUT` — the prompt and final response. It does **not** restore tool call arguments or results; those need `TRULENS_CAPTURE_TOOL_PAYLOADS`. Diffs need `TRULENS_CAPTURE_DIFFS` on top of that. `Tool Selection` and `Execution Efficiency` read the tool spans in the trace, so without `TRULENS_CAPTURE_TOOL_PAYLOADS` those spans carry tool names and timing but no arguments or output — the judge is reasoning over an emptied-out trace and the score means little. Turn on every capture flag you plan to evaluate against *before* the session runs; there's no reconstructing redacted content after the fact.
