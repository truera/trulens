...
def validate_items(items: list[dict[str, Any]]) -> None:
    for position, item in enumerate(items, start=1):
        ...
        if not isinstance(item["input"], str) or not isinstance(
            item["expected_output"], str
        ):
            raise ValueError(
                f"Dataset item {position} input and expected_output must be strings"
            )
        if not isinstance(item["metadata"], dict):
            raise ValueError(
                f"Dataset item {position} metadata must be an object"
            )
        if not isinstance(item["metadata"].get("source"), str):
            raise ValueError(
                f"Dataset item {position} metadata.source must be a string"
            )
