...
def evaluate_rag_response():
    ...
    # Comparison
    print("\n📈 COMPARISON:")
    print(f"   Groundedness difference: {results['comparison']['groundedness_diff']:+.1%}")
    print(f"   Relevance difference:    {results['comparison']['relevance_diff']:+.1%}")
    
    # Interpretation
    print("\n💡 INTERPRETATION:")
    if abs(results['comparison']['groundedness_diff']) > 0.1:
        direction = "higher" if results['comparison']['groundedness_diff'] > 0 else "lower"
        print(f"   Nuggetized evaluation shows {direction} groundedness,")
        print(f"   suggesting some facts have different support levels.")
    else:
        print(f"   Both methods show similar groundedness scores.")
    
    if abs(results['comparison']['relevance_diff']) > 0.1:
        direction = "higher" if results['comparison']['relevance_diff'] > 0 else "lower"
        print(f"   Nuggetized evaluation shows {direction} relevance,")
        print(f"   indicating varied relevance across answer components.")
    else:
        print(f"   Both methods show similar relevance scores.")
