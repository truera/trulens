...
1. To run GEPA optimization (generate new prompts):
   - Set RUN_OPTIMIZATION = True (around line 368)
   - Optionally change the category filter (line 371)
   - Run the script

2. To evaluate using saved prompts (skip optimization):
   - Set RUN_OPTIMIZATION = False (around line 368)
   - Make sure the prompt file exists (e.g., gaia_optimized_judge_prompts.json)
...
def log(message=""):
    ...
    # Uncomment the line below if you also want to see output in console
    # print(message)
...
# ============================================================
# 5. LOAD SAVED OPTIMIZED PROMPTS AND CREATE STUDENTS
# ============================================================
...
if RUN_OPTIMIZATION:
    # Original optimization code (kept for reference)
    optimized_prompts = {}
    optimized_students = {}
    ...
    for category, category_examples in grouped_examples.items():
        ...
        log(f"  Input trace (first 200 chars): {test_ex.trace[:200]}...")
        log("  Expected to output: 'critique' field")

        baseline_pred = student(trace=test_ex.trace)

        log("\n[DEBUGGING] What the LLM returns:")
        log(f"  Type: {type(baseline_pred)}")
        log(
            f"  Fields: {baseline_pred.keys() if hasattr(baseline_pred, 'keys') else 'N/A'}"
        )
        log(f"  Critique : {baseline_pred.critique}...")
