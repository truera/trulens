...
try:
    ...
    if user_input:
        ...
        conversations[0].render_message(
            new_msg,
            container=user_msg,
            key=str(len(conversations[0].messages))
        )
    ...
    def record_feedback():
        TOPIC_CATEGORIES = [
            "", "Technical", "Travel", "Personal advice", "Other"
        ]
        ...
        if not st.session_state.get("user_name"):
            st.warning(
                "Please login to persist your feedback",
    ...
    if st.session_state.pop("pending_feedback", None):
        st.toast(
            "Feedback submitted successfully", icon=":material/rate_review:"
        )
