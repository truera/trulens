...
class DBMigrationPreprocessor(VariableSettingPreprocessor):
    def __init__(
        ...
        super().__init__(timeout=timeout, kernel_name=kernel_name, code_to_run_before_each_cell=code_to_run_before_each_cell)
        shutil.copyfile(f"./release_dbs/db_compat_version/default.sqlite", "./default.sqlite")
...
for filename in listdir('./tests/docs_notebooks/notebooks_to_test/'):
    if filename.endswith('.ipynb'):
        ...
        for version in db_migration.migration_versions:
            ...
            setattr(
                DocsNotebookTests, f"test_db_backwards_compat_{test_version_str}_{filename.split('.ipynb')[0]}",
