...
```python
    ...
    from trulens_eval.tru_db import RecordInput, RecordOutput, RecordQuery
...
A selector is a dictionary from an argument name (like "text") to a `Query` object which indicates where in a `Record` to find values for the argment. Special `Query` objects are:

    - RecordInput -- names the main chain input/prompt.
    - RecordOutput -- names the main chain output/response.
    - RecordQuery -- names the root of the record. Can be further refined to select inputs/outputs of internal chain elements.
    - ChainQuery -- names the root of the chain. Can be further refined to select parameters of the chain or internal components.
  
An example defining a selector via a query is as follows, which selects the set of input documents that is part of a document-retrieval chain:

```python
    RecordQuery.chain.combine_docs_chain._call.args.inputs.input_documents
```

Further, as the above is sequence, we can indicate that a feedback method should be called on each of them by using slice notation in the query:

```python
    RecordQuery.chain.combine_docs_chain._call.args.inputs.input_documents[:].page_content
```

Finally, we can indicate how to aggregate multiple feedback method results with `Feedback.aggregate` which defaults to mean:
