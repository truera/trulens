...
- **Provider classes** -- `feedback.OpenAI` contains feedback function implementations
  like `qs_relevance`. Other classes subtyping `feedback.Provider` include
  `Huggingface` and `Cohere`.
...
- **Feedback implementations** -- `openai.qs_relevance` is a feedback function
  implementation. Here it is a method with the following signature:
    ...
    and a `statement`, both strings, and produces a float (assumed to be between
    0.0 and 1.0).

- **Feedback constructor** -- The line `Feedback(openai.qs_relevance)` constructs a
  Feedback object with a feedback implementation.

- **Argument specification** -- The next two lines, `on_input` and `on`, specify how
  the `qs_relevance` arguments are to be determined from an app record or app
  definition. The general form of this specification is done using `on` but
  several shorthands are provided. `on_input` states that the first argument to
  `qs_relevance` (`question`) is to be the main input of the app. The
  `on(Select...)` line specifies where the `statement` argument to the
...
a float indicating some quantity of interest. Any callable can be provided here
...
The mapping of record/app contents to the arguments to the specified feedback
implementation is provided via the `on_...` methods. These are discussed in a
later section.

### Global import requirement
...
page after selecting a record from a table.
...
As in the top example, a selector for a _single_ argument may point to more than
one aspect of a record/app. These are specified using the slice or lists in
key/index poisitions. In that case, the feedback function is evaluated multiple
times, its outputs collected, and finally aggregated into a main feedback
result.
