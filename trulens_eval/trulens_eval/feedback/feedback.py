...
class Feedback(feedback_schema.FeedbackDefinition):
        ...
        **kwargs
        ...
        if imp is not None:
            ...
            if "implementation" in kwargs:
                imp: ImpCallable = pyschema_utils.FunctionOrMethod.model_validate(
                    kwargs['implementation']
        ...
        if agg is not None:
            ...
            if kwargs.get('aggregator') is not None:
                agg: AggCallable = pyschema_utils.FunctionOrMethod.model_validate(
                    kwargs['aggregator']
                ).load()
            else:
    ...
    def _print_guessed_selector(self, par_name, par_path):
        if par_path == feedback_schema.Select.RecordCalls:
            alias_info = " or `Select.RecordCalls`"
        elif par_path == feedback_schema.Select.RecordInput:
            alias_info = " or `Select.RecordInput`"
        elif par_path == feedback_schema.Select.RecordOutput:
    ...
    def _default_selectors(self):
        ...
        if len(par_names) == 1:
            ...
            selectors = {par_names[0]: feedback_schema.Select.RecordOutput}
            self._print_guessed_selector(
                par_names[0], feedback_schema.Select.RecordOutput
            )
            ...
            # respectively.
            selectors = {
                par_names[0]: feedback_schema.Select.RecordInput,
                par_names[1]: feedback_schema.Select.RecordOutput
            }
            self._print_guessed_selector(
                par_names[0], feedback_schema.Select.RecordInput
            )
    ...
    def evaluate_deferred(
        ...
        shuffle: bool = False
    ) -> List[Tuple[pandas.Series, python_utils.Future[feedback_schema.
                                                           FeedbackResult]]]:
    ...
    def on_prompt(self, arg: Optional[str] = None) -> Feedback:
        ...
        if arg is None:
            ...
            self._print_guessed_selector(
                arg, feedback_schema.Select.RecordInput
            )

        new_selectors[arg] = feedback_schema.Select.RecordInput
    ...
    def on_response(self, arg: Optional[str] = None) -> Feedback:
        ...
        if arg is None:
            arg = self._next_unselected_arg_name()
            self._print_guessed_selector(
                arg, feedback_schema.Select.RecordOutput
            )

        new_selectors[arg] = feedback_schema.Select.RecordOutput
...
{q}
# or equivalently
{feedback_schema.Select.render_for_dashboard(q)}
```
...
"""
    ...
    def run(
        self,
        app: Optional[Union[app_schema.AppDefinition,
        ...
        **kwargs: Dict[str, Any]
        ...
        if self.if_exists is not None:
            if not self.if_exists.exists(source_data):
                ...
                feedback_result.status = feedback_schema.FeedbackResultStatus.SKIPPED
        ...
        try:
            ...
            if self.if_missing == feedback_schema.FeedbackOnMissingParameters.ERROR:
                feedback_result.status = feedback_schema.FeedbackResultStatus.FAILED
                raise e

            if self.if_missing == feedback_schema.FeedbackOnMissingParameters.WARN:
                feedback_result.status = feedback_schema.FeedbackResultStatus.SKIPPED
                logger.warning(
            ...
            if self.if_missing == feedback_schema.FeedbackOnMissingParameters.IGNORE:
                feedback_result.status = feedback_schema.FeedbackResultStatus.SKIPPED
                return feedback_result
        ...
        try:
            ...
            for ins in input_combinations:
                try:
                    result_and_meta = self.imp(**ins)

                    #cost += part_cost
                    # TODO: update cost tracking based on spans
    ...
    def _extract_selection(
        self,
        source_data: Dict,
        combinations: feedback_schema.
        FeedbackCombinations = feedback_schema.FeedbackCombinations.PRODUCT,
        **kwargs: Dict[str, Any]
