...
from trulens_eval.utils.generated import re_configured_rating
...
class GroundTruthAgreement(WithClassInfo, SerialModel):
    ...
    def agreement_measure(
        ...
        if ground_truth_response:
            ...
            ret = re_configured_rating(agreement_txt) / 3, dict(
    ...
    def absolute_error(self, prompt: str, response: str,
                       score: float) -> Tuple[float, Dict[str, float]]:
        """
...
class BenchmarkAggregator(WithClassInfo, SerialModel):
    ...
    def mrr(self, scores: List[float]) -> float:
        ...
        mean_reciprocal_rank = sum(reciprocal_ranks) / len(
            reciprocal_ranks
    ...
    def kendall_tau(self, scores: List[float]) -> float:
        ...
        Returns:
            float: Kendall's tau
        """
        tau, p_value = stats.kendalltau(scores, self.true_labels).correlation
