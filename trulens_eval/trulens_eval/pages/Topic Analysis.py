...
set_page_config(page_title= "Topic Analysis")
...
#Groundedness
st.subheader("% of queries by topic with failing Groundedness scores 🔴")
for col, topic, percentage in zip(st.columns(len(fail_percent_g)), fail_percent_g.keys(), fail_percent_g.values()):
    col.metric(label = topic, value = f"{round(percentage * 100, 3)}%")
...
st.selectbox(label = f"Topics with a failing percentage on Groundedness one standard deviation above the mean failing percentage: {round(avg_fail_percent_g * 100, 3)}%",
             options = list_stddev_away_g)

st.subheader("% of queries by topic with warning Groundedness scores ⚠️")
for col, topic, percentage in zip(st.columns(len(warning_percent_g)), warning_percent_g.keys(), warning_percent_g.values()):
    col.metric(label = topic, value = f"{round(percentage * 100, 3)}%")
...
#Answer Relevance
st.subheader("% of queries by topic with failing Answer Relevance scores 🔴")
for col, topic, percentage in zip(st.columns(len(fail_percent_ar)), fail_percent_ar.keys(), fail_percent_ar.values()):
    col.metric(label = topic, value = f"{round(percentage * 100, 3)}%")
...
st.selectbox(label = f"Topics with a failing percentage on Answer Relevance one standard deviation above the mean failing percentage: {round(avg_fail_percent_ar * 100, 3)}%",
             options = list_stddev_away_ar)

st.subheader("% of queries by topic with warning Answer Relevance scores ⚠️")
for col, topic, percentage in zip(st.columns(len(warning_percent_ar)), warning_percent_ar.keys(), warning_percent_ar.values()):
    col.metric(label = topic, value = f"{round(percentage * 100, 3)}%")
    
st.divider()

#Context Relevance    
st.subheader("% of queries by topic with failing Context Relevance scores 🔴")
for col, topic, percentage in zip(st.columns(len(fail_percent_cr)), fail_percent_cr.keys(), fail_percent_cr.values()):
    col.metric(label = topic, value = f"{round(percentage * 100, 3)}%")
...
st.selectbox(label = f"Topics with a failing percentage on Context Relevance one standard deviation above the mean failing percentage: {round(avg_fail_percent_cr * 100, 3)}%",
             options = list_stddev_away_cr)

st.subheader("% of queries by topic with warning Context Relevance scores ⚠️")
for col, topic, percentage in zip(st.columns(len(warning_percent_cr)), warning_percent_cr.keys(), warning_percent_cr.values()):
    col.metric(label = topic, value = f"{round(percentage * 100, 3)}%")
