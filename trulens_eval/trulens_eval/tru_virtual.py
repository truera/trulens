...
class VirtualApp(dict):
    ...
    def __setitem__(
        ...
        __name = feedback_schema.Select.dequalify(__name)
        ...
        for lens, call_or_calls in calls.items():
            ...
            for call in calls_list:
                ...
                if "stack" not in call:
                    path, method_name = feedback_schema.Select.path_and_method(
                        feedback_schema.Select.dequalify(lens)
                    )
...
class TruVirtual(mod_app.App):
    ...
    def add_record(
        ...
        if feedback_mode == feedback_schema.FeedbackMode.WITH_APP and record.feedback_results is not None:
